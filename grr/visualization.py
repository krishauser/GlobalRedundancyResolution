...
class GLRedundancyProgram(GLWidgetPlugin):
    ...
    def refreshResolution(self):
        ...
        if hasattr(self,'disconnectionDisplayLists_by_orientation'):
    ...
    def display(self):
        ...
        if hasattr(self.resolution,'delaunay'):
            ...
            assert len(ptset) == len(delaunay_pts),"Duplicate points! %d unique / %d"%(len(ptset),len(delaunay_pts))
            ...
            for s in delaunay.simplices:
                ...
                glEnd()
                """
                centroid = vectorops.div(vectorops.add(*[delaunay_pts[v] for v in s]),len(s));
                assert len(s)==3
                glBegin(GL_LINES)
                for i,v in enumerate(s):
                    neighbors = [w for j,w in enumerate(s) if i < j]
                    for w in neighbors:
                        if (v in delaunay_epts and w in delaunay_others) or (w in delaunay_epts and v in delaunay_others):
                            glColor3f(1,1,1)
                        else:
                            glColor3f(0,0,0)
                        glVertex3fv(vectorops.interpolate(delaunay_pts[v],centroid,0.1))
                        glVertex3fv(vectorops.interpolate(delaunay_pts[w],centroid,0.1))
                glEnd()
            glColor3f(0,1,0)
            glPointSize(5.0)
            glBegin(GL_POINTS)
        ...
        if self.drawWorkspaceRoadmap:
            ...
            if task.numConstraints > 3:
                ...
                self.disconnectionDisplayLists_by_orientation[mbest].draw(drawDisconnections,args=(mbest,))
            else:
